...
if($v->passes()) {
   ...
   if($result > 0){
    $success = 1;
    $sql = 'insert into tracking (order_status_id,note,order_id,items_no) values(?,?,?,?)';
    $result = setData($con,$sql,['6',$note,$order_id,$items_no]);
    ...
    sendNotification([$res[0]['s_token'],$res[1]['s_token'],$res[0]['c_token']],[$order_id],'طلب رقم - '.$order_id,"ارجاع الطلب - ".$note,"../orderDetails.php?o=".$order_id);
