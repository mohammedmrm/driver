...
if ($v->passes()) {
  ...
  $sql = 'update orders set order_status_id =?,new_price=? where id=? and driver_id=? and driver_invoice_id=0 and invoice_id=0 and locker=0';
  ...
  if ($result > 0) {
    ...
    sendNotification([$res[0]['s_token'], $res[0]['c_token']], [$order_id], ' طلب رقم ' . $res[0]['order_no'], "ارجاع الطلب - " . $note, "../orderDetails.php?o=" . $order_id);
  } else {
